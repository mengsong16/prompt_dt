...
def generate_one_subtask(env, policy, env_name, subtask_idx, input_traj_per_subtask, prompt_traj_per_subtask):
    ...
    print(f"================= ML45-{env_name}-{subtask_idx}-expert ==================")
    ...
    # set up save folder
    folder_name = f'ML45-{env_name}'
    folder_path = os.path.join(data_path, folder_name)
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    
    # save input trajectories 
    input_traj_file_name = f'ML45-{env_name}-{subtask_idx}-expert.pkl'
    ...
    prompt_traj_file_name = f'ML45-{env_name}-{subtask_idx}-prompt-expert.pkl'
...
def generate_ml45():
    ...
    ml45 = metaworld.ML45(seed=1) # Construct the benchmark, sampling tasks
    ...
    for name, env_cls in ml45.train_classes.items():
        ...
        sub_tasks = [task for task in ml45.train_tasks
                    if task.env_name == name]
        train_tasks.append(sub_tasks)
        train_envs.append(env)
        train_env_names.append(name)
    
    assert len(train_envs) == len(train_env_names) == len(train_tasks), "ML45: train envs, train_env_names, train_tasks should have the same length"
